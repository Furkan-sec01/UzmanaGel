...
struct FavoritesPage: View {
    ...
    var body: some View {
        ...
        .alert("Hata".localized, isPresented: $showError) {
            Button("Tamam".localized, role: .cancel) { vm.clearError() }
        } message: {
            Text((vm.errorMessage ?? "Bilinmeyen hata").localized)
        }
    }
}
...
private extension FavoritesPage {
    ...
    var emptyView: some View {
        VStack(spacing: 10) {
            Text("Favorin yok".localized)
                .font(.system(size: 16, weight: .semibold))
                .foregroundColor(Color("Text"))

            Text("Ana sayfadan kalp ikonuna basarak favori ekleyebilirsin.".localized)
                .font(.system(size: 13))
