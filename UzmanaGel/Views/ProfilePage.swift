...
struct ProfilePage: View {
    ...
    private var statsRow: some View {
        HStack(spacing: 12) {
            NavigationLink {
                ...
                statCard(value: reservationsCount, title: "Rezervasyonlarım".localized)
            }
            .buttonStyle(.plain)
            ...
            NavigationLink {
                ...
                statCard(value: favoritesCount, title: "Favorilerim".localized)
            }
            .buttonStyle(.plain)
        }
    }
    ...
    private var contactSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("İLETİŞİM BİLGİLERİ".localized)
    ...
    private var logoutSection: some View {
        ...
        .alert("Çıkış Yap".localized, isPresented: $showLogoutAlert) {
            ...
            Text("Hesabınızdan çıkmak istediğinizden emin misiniz?".localized)
        }
    }
    ...
    private func uploadProfilePhotoToFirebase(_ image: UIImage) async {
        ...
        do {
            ...
            print("UPLOAD ERROR (Storage), falling back to Firestore Base64:", error.localizedDescription)
            // Fallback to Base64 in Firestore
            guard let smallResized = resizeImage(image, targetMaxDimension: 250),
                  let smallData = smallResized.jpegData(compressionQuality: 0.6) else { return }
            ...
            do {
                ...
                print("UPLOAD ERROR (Firestore Base64):", error.localizedDescription)
            }
        }
    }
    ...
    private func loadUserData() {
        ...
        if let user = Auth.auth().currentUser {
            ...
            if let creationDate = user.metadata.creationDate {
                ...
                if months == 0 {
                    memberSince = "Yeni Üye".localized
                } else if months < 12 {
                    memberSince = String(format: "%d Ay Üye".localized, months)
                } else {
                    let y = months / 12
                    memberSince = String(format: "%d Yıl Üye".localized, y)
                }
            }
