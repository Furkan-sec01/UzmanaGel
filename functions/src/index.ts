...
export const sendReviewCreatedNotification = onDocumentCreated(
  "reviews/{reviewId}",
  async (event) => {
    const reviewData = event.data?.data();

    if (!reviewData) {
      console.log("Değerlendirme verisi bulunamadı.");
      return;
    }
    ...
    if (!providerId || !reviewId) {
      console.log("Değerlendirme bildirimi alanları eksik.");
      return;
    }

    const serviceTitle =
      typeof reviewData.serviceTitle === "string" ?
        reviewData.serviceTitle.trim() :
        "";
    ...
    const notificationId = await admin.messaging().send({
      ...
      notification: {
        ...
        body:
          `${displayServiceTitle} için ${ratingText} ` +
          "bir değerlendirme aldınız.",
      },
      data: {
        type: "review",
        reviewId: reviewId,
    ...
    console.log(
      "Değerlendirme bildirimi gönderildi:",
      notificationId
    );
  }
);
